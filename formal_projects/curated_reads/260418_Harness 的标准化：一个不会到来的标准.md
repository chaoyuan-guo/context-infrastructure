...
Chat Completions 成为事实标准靠两个条件（接口语义极简 + 厂商护城河不在接口层） → harness 属于运行时层，复杂度没有上限且各家解决的问题类别不同 → 运行时层的每项设计同时承载能力和壁垒（双面合同），共享必然撞上商业逻辑 → 连协议层的 MCP 都在因承载内容变复杂而分裂 → 真正收敛的标准出现在运行时层的上方（AGENTS.md）和下方（CLI + JSON），恰好落在无人有动力做私有扩展的位置。

核心论点：标准化的前提不是技术可行性，而是该层的利润结构是否允许共享。运行时层是 agentic 时代的利润中心，所以不会被标准化。

### 值得带走的

**双面合同**。运行时层的每项工程设计同时在做两件事：对外提供技术能力，对内构建商业壁垒。这个概念解释了为什么"技术上可以开源/标准化"和"商业上会去做"是两件不同的事。Claude Code 的八层防仿冒机制、Responses API 把推理链藏在服务端、Anthropic 切掉第三方 OAuth，都是双面合同的实例。它提供了一个可复用的判断框架：看一个技术层会不会被标准化，先看利润在不在这一层。

**标准化的位置迁移**。Chat Completions 时代的便利（写一遍到处跑）在 agentic 时代没有消失，而是从协议层迁移到了两个极端位置：最底层的 CLI + JSON（简单到没有可差异化的空间），最顶层的 AGENTS.md（只是一份 markdown 社会契约，不承担执行语义）。中间的运行时层一切都会被各家重新发明。

**Harness 对结果的影响量级**。同一个 Claude Opus 在不同 harness 里跑 SWE-bench 差出 16 个百分点，已经相当于一代模型升级的量级。这意味着选 harness 和选模型同等重要，而不是一个可以随意切换的工具选择。

### 行动指导

1. **集成点锚定在两条事实接口上**。构建 agent 工作流时，对外暴露的能力用 CLI + JSON，对 agent 的行为约束用 AGENTS.md 及相关文档。这两条接口跨 harness 可移植，中间的运行时层不要做跨厂商的抽象。

2. **按场景选 harness，不追求统一**。长会话深度思考用 Claude Code，批量并行任务用 Cursor Agent 或 Symphony，需要深度定制用 Codex CLI。同时用几个 harness 是按需切换不同维度的能力，不是混乱。

3. **用双面合同框架评估技术依赖**。在决定是否深度绑定某个技术层的某个厂商时，先判断这一层是不是该厂商的利润中心。如果是，预期它会越做越封闭；如果不是，它反而可能走向开放（参考 Kubernetes 的路径）。

### 存疑

文章最大的逻辑缝隙在于把当前竞争格局当作稳态。作者论证了"2026 Q1 三家领先者都没有动力开源 harness"，但把这个时间切片的结论推广成了"标准化不会到来"这个跨时间的判断。标准化往往发生在市场震荡之后（集装箱、TCP/IP），而非竞争最激烈的时候。如果模型能力趋同导致 harness 层变成 commodity，或者某家模型拉开代差后主动开源 harness 来扩大分发，双面合同的前提就被改写了。

作者对开源 harness 框架的回应（"追赶者才开源"）也经不起推敲。Kubernetes 来自容器编排的领先者 Google，Llama 来自 AI 研究的顶级玩家 Meta。它们开源的动机不是"落后"，而是"利润不在这一层，commoditize 这一层对自己有利"。用作者自己建立的双面合同框架恰好可以推出这个推论，但作者在回复评论时没有把自己的框架用到底。

四层模型本身也值得追问。协议层和运行时层的边界在实践中未必清晰，MCP 的 *meta 扩展就模糊了这条线。如果某些 harness 功能可以被下推到协议层（变得足够简单从而可标准化），"运行时层不可标准化"的结论就需要重新划定范围。
