...
> 来源：<https://www.superlinear.academy/c/ai-resources/ai-software-engineering>
> 作者：Yan Wang 鸭哥
> 发布日期：2025-11-17
...
半年前，我写了[这篇文章](https://yage.ai/claude-code.html)。当时有两个论断：Claude Code虽然刚发布没人重视，但这种方便集成的的Agentic能力很重要；未来软件分发和开发的方式会有很大变化，Library as a Service会成为一种重要的新方式。这半年来，我一直在实践这些思考，成为了Claude Code类工具的[重度用户](https://yage.ai/wide-research.html)，也为我们的AI课程开发了自己的LaaS产品。
...
1. 提供精细原始的反馈 传统API在捕获底层错误后，一般会抛出一个抽象的高层异常。这对于AI的Agentic工作流是致命的。AI的有效性依赖于一个[尝试-反馈-修正的循环](https://yage.ai/agentic-ai-crisis.html)。不论是对人还是对AI，模糊的“操作失败，请稍后再试”的错误信息会直接中断这个循环，让人无所适从。 因此，对AI原生的软件，我们要提供富含技术细节的原始错误信息。当封装异常时，尽量保留完整的上下文。比如宁愿rethrow一个带stacktrace的ConnectTimeoutError，也不要重包装成一个通用的APIFailureError。前者AI一看就知道哦某个特定步骤网络超时了，后者得从模糊的自然语言里面推断错误的底层原因，效率低很多。精细原始的反馈，是提升生成效率和意图保真度的基础。
2. 暴露细粒度的控制 为了降低人类的学习和误用成本，传统API设计一般会隐藏底层的、细粒度的接口。但这个约束对AI完全不适用，它可以3秒看完100页文档，学习成本几乎为零。相反，这些低级接口是生成内核表达范围的直接保障。当高层抽象没办法满足用户的长尾需求时，AI可以利用这些细粒度的接口进行自由组合和微调，创造出标准接口实现不了的解决方案。
3. 交付足够详细的知识 传统软件的文档是写给人看的，是代码的外部参考，总觉得比代码低一头。但生成内核中的引导知识，是写给AI看的，是交付物本身的一部分，是一等公民。因为过去需要开发者通过数年经验才能内化的最佳实践、设计哲学，现在可以被系统性地编码成Prompt，作为内核的一部分一同交付。想象一下一个AI看没看过Effective C++，写出来的代码质量是不可同日而语的。这些知识是提升生成效率和意图保真度的强大杠杆。
...
“我并没有有意识地避免重复工作，甚至有意违背了软件工程最核心的原则——DRY（Don't Repeat Yourself，不要重复自己）。”最近高强度的vibe coding让我也是对这一点深有同感！我有很少一部分的代码，一开始是特意抽象出来了做成一个框架，后续重复利用率也非常高。每次当我想这么做的时候，就会想很多事情，导致起步很慢，最后为了赶ddl不得不写一个简单但是可复用性低的代码。不过总体体验下来，感觉还是时间越长，代码可复用性的重要性就越是明显。
...
这样带来的是软件的边际成本极低 竞争激励 这种市场下的商业模型感觉也会发生巨大变化

**↳ Yan Wang 鸭哥** *2025-11-21*
...
最近我一直在思考什么是 AI Native Development，而鸭哥的内容恰好把许多模糊的想法一下子具象化了。

在日常使用 AI 进行编程的过程中，我越来越明显地感受到：未来的 AI 原生开发将与传统的软件开发范式产生本质性的差异。

比如，过去在人类主导的流程中，文档、单元测试和代码重构往往因为时间和成本限制而难以做到充分；但在 AI 时代，这些内容反而成为帮助 AI 更好理解和维护代码的重要“上下文资产”。

通过让 AI 自动生成文档、补齐测试覆盖率、持续执行重构，项目能够在整个生命周期中保持更清晰的结构和更高的可维护性，也能让 AI 在后续迭代中发挥更高的效率。

因此，AI 原生开发的核心不在于某个具体工具，而在于一个新的原则：**假设 AI 可以从头到尾独立完成任务，而人类的角色是帮助 AI 更好地完成工作。**

人类不再是主要的编码者，而是负责构建高质量的上下文、结构化的信息和明确的目标，并将各种辅助工具和自动化流程整合起来，让 AI 能够精准、高效地执行任务。

最近，在和一位比我更资深、经验更丰富的开发者合作时，我发现：**资深开发者可能更容易被传统经验束缚，把 AI 当作辅助工具，而不是把自己视为 AI 的辅助者。很多场景下还是自己去亲力亲为。**

这种角色认知的差异反而限制了整体开发效率。

因此，看似只是一个底层思维的小小转变，却可能给开发效率和质量带来巨大的提升。

**↳ Alton LEE** *2025-11-20*

我似乎贡献了这思维的转变，当然只因为 AI 所以才变为可能。我不是开发者，但加入进来发现我原来蛮荣幸可以在这里。

**↳ Yan Wang 鸭哥** *2025-11-21*

是的，而且我越来越感觉类似的变化是有规律可循的，不是孤立的。最近也在一直反思。
...
鸭哥所说的「交付形态」和目前「Claude Skill」似乎是类似的东西，这样理解对吗？

**↳ Yan Wang 鸭哥** *2025-11-19*

我觉得需要更specific一些。我先打个比方，比如我们出门想买汽油，需要一个容器来装。你可以用矿泉水瓶来装，可以用洗菜的盆来装，也可以用水桶来装。这些都是兼容的，它一定程度上可以完成装汽油的任务，但是它都不是原生的。最好用的还是加油站那种接地的油桶。

我们这里也是一样，你可以用Claude Skill来作为这种UGS软件的交互形态，可以用MCP，可以用Cursor Rules加命令行，但它们都不是原生的。
...
更specific指的是把知识fine tuning进去？ 但是这样的话大部分人都做不了

**↳ 村里小伙** *2026-02-10*

它应该比编程语言要简单很多，但比传统API要更加细，这种又复杂，又没那么复杂的东西怎么让模型内化呢

**↳ Yan Wang 鸭哥** *2026-02-14*

不是，这绝大多数场景里其实fine-tuning并不是使用LLM的一个很好的方法。事实上fine-tuning也没办法ingest知识，它更多的是激活LLM内部已有的一些能力或者知识。
...
最近还看到一个git项目oraios/serena，把LSP语言服务器作为工具直接给AI，让AI可以获得更简介、更语法层面的信息；而不是靠文件搜索。

**↳ Yan Wang 鸭哥** *2025-11-17*
...
我在想，DRY之所以这么有用，是因为它指引人去把时间用在构建系统而非运营上，选择长期，牺牲短期，所以长期来看会赢。

鸭哥其实也是这样的原则，只是背后的参数、环境，变了。
...
感谢分享,很有启发! 也分享一个我的联想和大家讨论 

- 正是因为UGS以后纯提供软件的软件公司 即使继续存在他们的profit margin也会被压缩
- 会持续增长的提供generative kernel的公司将是那些提供软件之外的solution的公司（比如说financial security、Regulation、或者是与现实世界的接口）或者是更底层的networking、storage（eg. Firebase)
...
上次调试的时候，发现喂给chatgpt二进制的内存块都很有效果，以后LLM可以不仅读stacktrace，连内存也一并读了，还有寄存器的状态等
...
非常同意，DRY 原则下最优的可复用模块（轮子）的规模取决于 复用强度*造轮子成本 / 抽象的固定成本。 Agentic coding 下造轮子成本降低了，那么就不需要做这么多抽象了
...
深同感受，虽然我不是软件开发者，但通过提到的例子，我好像更明白了AI对软件工程的影响。
...
嗯之前确实没从ai native的软件工程角度思考过问题。 另外codeing agent对应ugs是不是就像视频软件之于content creator，最近感觉研究agent的本身的技巧带来的提升在变得越来越小了
...
非常赞同下一代library/API“提供富含技术细节的原始错误信息，当封装异常时，尽量保留完整的上下文“。 如果这些信息是给人类读的，不能搞太长，但是library/API for AI 的就不再是这样了。
