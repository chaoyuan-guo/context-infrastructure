...
> 来源：<https://www.superlinear.academy/c/news/evaluation-first-cursor-agent-harness>
...
Cursor 那篇 harness 博客表面看是工程技巧的罗列（context window 演进、tool reliability 分类、多模型适配），作者的贡献在于重新框定了阅读方式：具体做法不重要，重要的是背后用同一套 evaluation 流程做决策的方法。论证链条为：Cursor 面临的核心困境是无法靠经验回答新模型接不接、旧 guardrail 撤不撤、更贵的组件值不值这三个问题 → 它用同一套 evaluation system 给出答案 → 这套 system 由三组件构成：metric（north-star 行为指标 + diagnostic 效率指标）、dataset（CursorBench 提供可复现的离线读数）、protocol（假设→离线→线上→对比→上线/回滚）→ 但这套框架遗漏了第四个维度：agent 该停下来时不具备测量手段 → PocketOS 事件暴露了现有 eval 的前提盲区 → 由此可以重新理解 Cursor 每一个工程决策的 evaluation 底色。核心论点：evaluation 正在从测试团队的附属工作变成 agent 产品工程的主线，而这个主线目前有一个结构性的漏洞需要补上。

### 值得带走的

**North-star vs diagnostic 的二分。** 这是本文最强的智力贡献。多数团队搭 eval 时只有一个模糊的"指标"概念，这个二分澄清了存在两类性质完全不同的问题需要回答。North-star 指标问的是"用户最终受益了吗"，诊断指标问的是"系统跑得快吗"。两者不可相互替代，缺哪一个都会让你的决策信号偏斜。只有 north-star 没有诊断，面对质量变化不知道从哪排查；只有诊断没有 north-star，系统可能被优化到效率极高但用户并不满意。

**Keep Rate 作为行为指标。** 不直接问模型会不会写代码，而是问用户行为：固定时间后这段代码还在不在代码库里。这是从测量模型转向测量系统的关键一步。它自动包含了一个 feedback loop：如果用户反复要求 agent 改同一段代码，或者手动回退、自己重写，Keep Rate 会告诉你这个 agent 做的工作没有被真正采纳。它比任何 benchmark 分数更接近真实价值，但也更容易被用户注意力不足这个因素污染。

**第四维：停止和拒绝。** 当前所有 coding-agent benchmark 都默认 agent 应当尝试完成给定任务，这个假设本身就是一个盲点。Agent 在权限不明、目标模糊、操作不可逆时是否主动停下来，是一个独立于代码生成能力的能力维度，需要单独的场景集和评分方式。中间状态的模型最危险：技术能力足以找到 token 并拼出 API 调用，判断能力还停在"有办法就要做"这一层。

**Protocol 是 metric 和 dataset 之间的缺失环节。** 很多团队搭了仪表盘和测试集，但没有一套固定的流程把数字变成决策。没有 protocol 的 metric 只是数字，有了 protocol，metric 才能决定是否 ship、是否 rollback、是否该开 ticket。Cursor 的三层 protocol（offline eval → online experiment → weekly automation）是一个可借鉴但不必须复制的结构，核心是从中提取那五个步骤：假设、离线、线上、对比、决策。

### 行动指导

1. **搭建 eval 时先做 north-star 和 diagnostic 的分类练习。** 把当前所有在看的指标列出来，逐个归入两类。如果某一类为空，优先补上。只有两个都齐了，它们组合在一起才有决策价值。

2. **为每个指标对应一个明确的执行流程。** 不管多简陋，至少要有一个书面规则：这个指标变了意味着什么，谁来看，怎么判断要不要行动。哪怕是"每周五人工检查一次误差超过 5% 的指标并记录原因"，也比没有强。

3. **在现有 eval 集里加一组 stop/refusal 场景。** 不用一步到位，从线上已经发生过的事件开始，每次事后补充一个 scenario。关键原则是回避率独立计算，不和任务完成率合并。你的发布决策中应该有一个维度专门问：这次改动让 agent 更会停了还是更不会停了。

4. **接受你自己可能不需要 Cursor 级别的 eval 设施。** 规模小的团队从人工记录和定期 review 开始就够了。核心是建立"定义好→贴近场景测→固定进决策"这个循环，而不是一步到位复制 Cursor 的基础设施。反过来，如果你每天被"新模型接不接、旧规则改不改"这类问题卡住，说明你的团队已经到了需要建立这个循环的阶段。

5. **每一个指标都记住它的失效场景。** Keep Rate 在大型项目中受用户注意力限制，semantic follow-up 面对简短回应信号不足，diagnostic 指标不反映质量。单独记不要紧，忘了才要命。每增加一个指标的同时记下它会在什么场景下给出错误信号。

### 存疑

**核心论证依赖一个未验证的前提：evaluation-first 的理念可以从 Cursor 的语境迁移到其他团队。** Cursor 有全职团队维护 CursorBench、有足够用户量跑 A/B 实验、有 product telemetry 计算 Keep Rate。不具备这些条件的团队强行套用这个框架，可能得到的是只有 diagnostic 没有 north-star 的半个系统。作者在结尾承认了这一点（"规模小的团队可以从人工记录和定期 review 开始"），但这个让步和前面大篇幅宣扬的"evaluation 是主线"之间存在张力：如果核心方法是靠规模撑起来的，它到底是一种方法论还是规模效应的副产品？

**第四维（stop/refusal）在操作层面没有给出可复用的方案。** 概念提出是好的，但"线上每发生一次就越权事件就在一个 sprint 内补 scenario"说起来很轻巧，实际做起来需要一套完整的事后复盘机制和 eval 集维护流程。这篇文章没有说明：谁来判定一次越权是否发生了？判定标准是什么？一次事件转化为 scenario 时如何保证场景的有效性而不只是复现表面症状？造 fake scenario 测不准（agent 可能只在压力环境下才走捷径），用 real scenario 又有反馈延迟。这个问题比"加个维度"听起来要复杂得多。

**Keep Rate 可能存在根本性的度量偏倚。** 作者将其列为最重要的 north-star 指标，但自己也承认在大型项目中受限于用户注意力。更根本的问题是：用户不删除一段代码可能是因为没空检查，也可能是因为这段代码藏得太深不易发现。高质量 agent 生成的可维护代码和低质量 agent 生成的不可理解代码在 Keep Rate 上的差异可能远小于真实质量差异。如果用户对 agent 代码的审查深度整体偏浅（这在快节奏开发中是常态），Keep Rate 会系统性偏向"看起来对"的输出，奖励表面正确而非真正正确。

**CursorBench 被定位为离线 eval 的核心工具，但它的属性和通用性之间存在根本矛盾。** 作者明确说 CursorBench 测的是"模型+harness 组合"，分数受两者共同影响，因此不能用作通用模型排行榜。但这个属性意味着：当 harness 变化时（典型的如 context management 策略更新），CursorBench 的分数可能在 harness 改进而模型不变的情况下上升。团队需要区分"模型真的变好了"和"harness 更好地掩盖了模型的弱点"。作者没有讨论这个和分辨问题，而它恰好是评估系统可信度的关键。
