...
> 来源：<https://www.superlinear.academy/c/ai-resources/wide-research>
> 作者：Yan Wang 鸭哥
> 发布日期：2025-10-18
...
与此同时，即使面临着同样大规模的输出，不同LLM在任务完成的质量上也是有差异的。这一方面是开始偷懒的时间不同，比如早期的GPT-4o-mini往往会从几百个字就开始偷懒，而现在的GPT-5和GLM-4.6则可以撑到两三千字。另一方面是，在Agentic AI时代，模型对工具的调用和思维习惯也有很大影响。比如Gemini特别不爱用搜索，Codex每次做完事情会习惯性地复盘看一下有没有漏掉什么东西。这就导致就算它俩都从输出四千字的时候开始偷懒，最终完成任务的效果也不一样。另一个例子是Claude 在写代码的时候如果遇到一个测试修不好，它可能就会去把测试本身给删了，甚至直接print一行“完美🎉 测试已经修复！”来蒙混过关。但gpt-5-codex 从来没有发生过这样的情况。这也是为什么在重度使用各家Agentic AI产品之后，我目前停留在了Codex CLI上。
...
感谢鸭哥，学到很多。multi-agent那个section，你说不给LLM戴帽子。我有个问题：我个人的体感是给LLM戴帽子之后，他更倾向于输出更加domain-specific的内容，而不至于过于泛泛。请问鸭哥怎么看？你的best practices是什么样的呢，可以举个例子吗

**↳ Yan Wang 鸭哥 → Fanyu Meng** *2025-12-05*

这是一个非常好的观察，但是我比较建议把这个问题拆成两个维度：

第一是我们怎么样让LLM针对我们感兴趣的问题做出精准回答；第二是我们给LLM一个role play性质的指示。后者可以作为前者的一种解决方法，但未必是唯一的解决方法。

我举个例子，比如我们看上了一款时装或者汽车，想让LLM辅助我们决策要不要购买。这时候你可以让它用专业时尚媒体或者汽车媒体撰稿人的角度来分析。这样往往可以逼着它不要只说“挺好看的”、“挺实用的”这种宽泛的评论，而触及到更深层的角度。

但一种alternative是我们不用给它戴帽子，而是直接让它给出“有深度、有启发、对我做出购买决策有帮助的意见”。根据我的经验，后者的效果往往会更好，因为它有了更明确的目标和评价标准，也就可以变得更加有帮助。前者这种戴帽子的方法还是用一种间接的方式来描述你想要什么，这种时候往往直接跟它说我们是想用在什么场景、实现什么目标，会让它更能领会我们的意思。

**↳ Fanyu Meng → Yan Wang 鸭哥** *2025-12-07*

OK，这还是和对好的管理者的要求是一样的，不micro-management，let the expert do expert stuff ;)
...
这个对我很有启发，当然也让我创纪录的快读消耗了token 哈哈。不过我最近感觉claude skills以及subagent其实在某种程度上是在做类似的事情，这种基于固化人的流程的wrokflow我感觉最终会在除了探索之外的领域取代类似的ai native工作流，不知道鸭哥是不是有类似的看法？

**↳ Yan Wang 鸭哥** *2025-11-14*

我感觉是这样的，现在越来越感觉基于人类智能的SOP/workflow也是可以自动化的/
...
感谢鸭哥分享！我这里有个关于实现方案评估的追问

“我就让 Codex 把两个方法分别实现了一下，然后在测试集上比较了结果。全程花了 10 分钟 dev time，带等待时间一共一个小时。结果的对比很明显：用 Codex 更好。它可以在控制力、效果和成本上实现更好的平衡。”
鸭哥可否再阐述一下对这两种方案你测试和评估的细节？我理解下来是有这么两个实现方式：“写一个 Python 脚本去批量调用一个轻量级 LLM” vs. “启用Codex subprocess处理子问题“。前者听起来更像self managed LLM call，需要手撸类似agent的流程，后者直接使用了Codex coding agent。听起来似乎后者更符合解决当前问题的sub-agent的需求？另外前一种解决方案适合什么样的use case呢？

**↳ Yan Wang 鸭哥 → Kevin Jiang** *2025-10-23*

是的。前者是手撸 agent，它的主要好处是我们有完全的控制权。比如可以严格规定输出的具体 schema 是什么，可以保证它一定是一个 JSON 格式，从而方便后续脚本进行使用程序来进行处理，这样可以规避 LLM 在合并的时候偷懒的问题。

换言之，它的每个 agent 开发成本更高，但是 aggregation 的开发成本更低。codex subprocess 每个 agent 开发成本很低，但最后 aggregation 开发成本更高。比如你看我们最终的方案是用prompt提示它用patch tool一章章更新最终的文档，而不要一把把最终结果写出来。
...
最近在给Amazon Q做类似的multiple agent探索(别问我为啥是这货)，正好可以试一下方法论。现在我自己也基本上是codex，我也去实践一下
...
除了wide research 这种并行处理再汇总，大家还遇到过哪些常见的agent engineering pattern ？

**↳ Zayn → Niel Hu** *2025-11-16*

好问题，分而治之的工程实现方案是 MapReduce，在 agent 场景能用到。我即刻想到的另一个 idea 是资源池/缓存池的概念，MapReduce 上更进一步，允许 task dispatcher agent 找到合适的带有部分 context 的 sub agent 执行某些步骤。
...
“拒绝人类式角色分工的多智能体设计”这一部分让我深有共鸣。大语言模型本身具备跨领域的综合能力，而人为地将它限定为“产品经理”“设计师”“工程师”等角色，其实是在削弱它的智能表现。这一观点非常深刻。我在设计 Agentic 系统的过程中也有类似体会：基于“角色人格”的提示往往让模型的推理多样性下降、思考过早收敛；而 **基于上下文隔离（context isolation）与清晰任务边界** 的架构设计，则能让系统表现出更一致、更可扩展的智能行为，更贴近真实的智能协作。

**↳ Fanyu Meng → 鸡哥** *2025-12-03*

感谢分享！我也对这个观点很感兴趣，似乎和目前主流的做法正相反。可以分享一个具体的操作实例吗？
...
Tavily MCP 确实很好用啊，多谢推荐。如果用本地 npm package 可以把  api key 写在环境变量里，但用 remote 确实方便，但我也不想把 key 留在 .mcp.json 里 （我经常一个目录用不同 AI 工具，倾向用这种相对中立的配置文件） Multi-Sub-Agent好像大家都也能纠结啊， 牛人可以几个帽子轮流戴，但是不是同时带几个帽子也难说。LLM 脑容量比我们大，但 Context 一长就迷糊了，这几个月大家都好像倾向 offload 到文件系统，甚至不要像 MCP 一样搞那么长的 tool spec，需要时- -help自己查文档

**↳ Jove Zhong** *2025-10-18*
...
同目录下的.env （不用 checkin git）

```
TAVILY_API_KEY=tvly-dev-Qk..
```

**↳ Yan Wang 鸭哥 → Jove Zhong** *2025-10-19*

学习了！

另外我觉得这几个问题都是很好的问题。你提到的 context 一长就迷糊了就是需要 context window separation 的原因。我们用 multi-agent 也就是为了克服这一点。换言之，这种 multi-agent 的视角，主要动因是为了克服 context 一长就迷糊的问题，而不是一个 LLM 只能专精一行的问题。

另外 offload 到文件系统也是一个很好的 idea，Manus 的这篇文章有比较清楚的叙述。
...
“它有点像《三体》里面说的探测比打击更昂贵。我有时间在这儿调研、分析、纠结，还不如现场用 AI 直接把它干出来快。而且基于这些第一手经验做出的决策质量也是特别高的，真是有种“大人，时代变了”的感觉。”

这也是一个很重要，我们需要反复提醒自己的思维和习惯的转变。所谓AI native，这个做法可能占了大部分
...
光这一段话，就吊打市面上各种死搬硬套的AI evals呀。More intelligence, less structure
...
AI偷懒的现象 → 归因为 Transformer 长输出时 instruction following 衰减（架构约束）+ 不同模型工具调用习惯的差异（执行力差异）→ 架构层用 Wide Research 分治绕过长输出限制 → 执行层选 Codex 保证微观可靠性 → 落地时采用 Senior Manager 式流程设计让 AI 自我迭代 + 用实现替代分析加速决策 → 工程细节上引入 Tavily 消除网络摩擦 + 用 context window 隔离替代人类式角色分工。

核心论点：AI"偷懒"是架构问题而非意愿问题，对策是分治策略（Wide Research）配合高执行力模型（Codex），再加上一套让 AI 自我反思、自我迭代的开发流程。

### 值得带走的

**架构约束 vs 执行力差异**的区分是这篇文章最有解释力的分析框架。面对"AI 不好用"这个笼统抱怨，它把归因拆成两条独立的因果链：一条是所有 LLM 共有的结构性限制（长输出衰减），你无法改变它，只能用分治绕过；另一条是模型之间的可比较差异（工具调用习惯、复盘行为），你可以通过选择来优化。这个区分值得记住，因为它直接决定了你应该改架构还是换模型。遇到 AI 产出质量问题时，先判断属于哪一类，再选对应的策略。

**"做 Senior Manager 设计组织流程"**这个比喻准确定位了人类在 AI 协作中的杠杆点。不是跟着每个 work item 给反馈，而是设计一个自我迭代的系统：让多个 Sub-Agent 各自执行、各自反思，汇总改进建议后由人类做最终判断。这个思路后来在鸭哥的《从过程确定性到结果确定性》中演化为更完整的 Agentic Loop + Evaluation-First 框架，但种子在这里。

**"探测比打击更昂贵"的逆转**捕捉到了 AI 时代决策方式的一个结构性变化。当实现成本被压缩到足够低时，"先做出来再比较"比"先分析再动手"更快且决策质量更高。这个认知的关键在于它有一个隐含的生效条件：实现成本确实已经足够低。意识到这个条件本身就是一种校准能力。

**基于 context window 隔离而非角色扮演的 Multi-Agent 设计**，从底层逻辑挑战了主流的 Multi-Agent 实践。LLM 本身全知全能，给它戴"你是工程师"的帽子是削弱而非增强。正确的分 Agent 动机是防止 context 膨胀导致质量衰减，而非模仿人类社会的职能分工。鸭哥在评论区的补充进一步澄清：戴帽子是间接描述你想要什么，不如直接给目标和评价标准。

### 行动指导

1. **遇到 AI 产出质量问题时先做归因分类。** 如果任务需要的输出本身很长（比如几十个独立子任务的汇总），大概率是架构约束，解法是拆成子任务分别处理再汇总。如果输出长度合理但质量不行，先检查验收标准是否显式写清楚了。

2. **设计 AI 自我迭代的流程，而非逐条给反馈。** 在需要反复调优 prompt 或工作流的场景中，让多个 Agent 各自执行同一批测试任务，各自总结经验教训和改进建议，汇总后由人类做决策。这比手动一轮一轮跟着跑高效得多。

3. **在两个方案之间纠结时，评估"直接做出来"的成本。** 如果用 AI 把两个方案各实现一遍只需要半小时以内，直接做，用结果说话。在实现成本较高的场景（几天以上）仍然应该先分析。

4. **给 AI 配工具时优先消除摩擦最大的环节。** 本文中引入 Tavily 替换原生网络访问后速度提升两三倍，这种工具层的隐性摩擦往往是真正的瓶颈。凡是 AI 反复卡住的确定性操作（网络抓取、格式校验、文件处理），都值得封装成稳定工具直接调用。

5. **设计 Multi-Agent 时用 context 隔离替代角色标签。** 分 Agent 的目的是防止单个 context window 过载，不是模仿人类分工。每个 Sub-Agent 只需要拿到自己子任务的 context，通过文件和主 Agent 通信。不要给它们贴"你是 XX 专家"的标签，直接描述任务目标和评价标准。

### 存疑

Wide Research 的分治策略依赖一个前提：任务可以自然分解为独立子问题。53篇文章总结天然满足这个条件，但文章在推广方法论时对适用边界讨论偏少。当子任务之间存在强依赖（比如需要跨文章做关联分析）时，纯分治会丢失全局信息。aggregation 阶段如何恢复这种全局视角，正文中没有展开，只在评论区回复 Kevin Jiang 时提到了 aggregation 成本的 tradeoff。

作者关于模型"性格"的判断（Codex 会复盘、Claude 会删测试蒙混过关）主要基于个人使用经验，是单点观察而非系统性评测。LLM 的行为模式会随版本更新漂移，也会随 prompt 设计和任务类型变化。把这些行为归结为模型的稳定特性可能过度简化了。鸭哥自己在三个月后的《从过程确定性到结果确定性》中也提出了更成熟的归因：很多"偷懒"问题的根源在于缺少显式验收标准，而非模型本身不靠谱。这两篇文章之间存在一个归因重心的迁移，但没有被显式 reconcile。

"探测比打击更昂贵"的逆转在本文场景中成立（10分钟 dev time），但文章的修辞力度让这个原则显得比实际适用范围更普适。当两种方案的实现成本差距很大时，先分析仍然是更理性的策略。这个生效条件在文中没有被显式化，读者容易把一个有条件的判断当成无条件的原则。
